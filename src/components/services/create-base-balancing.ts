...
export abstract class CreateBaseBalacing {
    ...
    protected abstract getTypeCategoryName(): string
    ...
    private async showCategory(): Promise<void> {
        const result: CategoriesIncomeType[] | DefaultResponseType = await HttpUtils.request(this.getCategoryUrl());
        (result as CategoriesIncomeType[]).forEach((expenseCategory): void => {
            ...
            this.typeCategoryName = this.getTypeCategoryName()
        });
    ...
    private handleTypeChange(e: Event): void {
        const selectedValue: string | null = (e.target as HTMLSelectElement).value
        if (selectedValue === '1') {
            this.openNewRoute('/balancing/create-income').catch(error => {
                console.error('Ошибка при загрузке категорий доходов:', error);
            })
        }else if (selectedValue === '2') {
            this.openNewRoute('/balancing/create-expense').catch(error => {
